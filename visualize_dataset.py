...
def visualize_synthetic_data():
    ...
    for i in range(3):
        # گرفتن عکس و مختصات از دیتاست
        img_tensor, coords_tensor = dataset[i]
        
        # تبدیل تنسور به عکس قابل نمایش
        img_np = (img_tensor.permute(1, 2, 0).numpy() * 255).astype(np.uint8)
        img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
        h, w = img_np.shape[:2]
        
        # تبدیل مختصات نرمال‌شده به پیکسل واقعی
        coords = coords_tensor.numpy() * np.array([w, h])
        
        # رسم نقطه‌ها و خطوط دور کاغذ
        for (x, y) in coords:
    ...
    print("✅ Saved 'synthetic_samples.jpg'. Check the image!")
    plt.show()

...
def compare_dropout_effect():
    ...
    print("Loading weights (No Dropout)...")
    model_no_drop = DirectCornerRegressor(use_dropout=False).to(device)
    model_no_drop.load_state_dict(torch.load("weights/direct_corner_best(new).pth", map_location=device))
    model_no_drop.eval()
    
    with torch.no_grad():
        coords_no_drop = model_no_drop(tensor).squeeze().cpu().numpy()
    ...
    print("Loading weights (With Dropout)...")
    model_with_drop = DirectCornerRegressor(use_dropout=True).to(device)
    model_with_drop.load_state_dict(torch.load("weights/direct_corner_best(dropout-new).pth", map_location=device)) 
    model_with_drop.eval()
    
    with torch.no_grad():
        coords_with_drop = model_with_drop(tensor).squeeze().cpu().numpy()
    ...
    axs[0].set_title("1. Model A (No Dropout)", fontsize=16, fontweight='bold')
    axs[0].axis("off")

    axs[1].imshow(img_with_drop)
    axs[1].set_title("2. Model A (With Dropout)", fontsize=16, fontweight='bold')
    axs[1].axis("off")
